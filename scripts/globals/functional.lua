...
-- e.g: fn.filter({"a", "b", "c", "d"}, function(k, v) return v >= "c" end)  --> {"c","d"}
...
-- e.g fn.filterArray({ "a", "b", "c", "d" }, function(k, v) return v >= "c" end)  --> { 1 => "c", 2 => "d" }
...
-- e.g: fn.sum({ 1, 2, 3 }, function(k, v) return v end)  --> 6
