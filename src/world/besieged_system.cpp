...
{
    // Iterate through all strongholds, excluding alzhabi, and update forces
    // TODO: Iterating through enums is not a good practice and can be error prone
    // For now, this is fine since these will not change often, but consider a better method
    for (int i = BESIEGED_STRONGHOLD::MAMOOK; i <= BESIEGED_STRONGHOLD::ARRAPAGO; i++)
    {
        auto strongholdInfo = this->besiegedData->getBeastmenStrongholdInfo((BESIEGED_STRONGHOLD)i);
