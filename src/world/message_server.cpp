...
using InternalHandler = std::function<void(std::vector<uint8> const&, in_addr, uint16)>;
...
void message_server_init(WorldServer const& worldServer, const bool& requestExit)
{
    ...
    regionalMsgHandlers[REGIONALMSGTYPE::REGIONAL_EVT_MSG_CONQUEST] = [&worldServer](std::vector<uint8> const& bytes, in_addr from_ip, uint16 from_port)
    {
